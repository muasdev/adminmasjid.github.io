...
$berita_view = NULL; // Initialize page object first

class cberita_view extends cberita {
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["id"] <> "") {
				...
				$sReturnUrl = "beritalist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "beritalist.php"; // No matching record, return to list
					}
			}
		} else {
			$sReturnUrl = "beritalist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($berita_view)) $berita_view = new cberita_view();

// Page init
$berita_view->Page_Init();

// Page main
$berita_view->Page_Main();
...
var CurrentForm = fberitaview = new ew_Form("fberitaview", "view");

// Form_CustomValidate event
fberitaview.Form_CustomValidate = 
...
fberitaview.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
<?php
	foreach ($berita_view->OtherOptions as &$option)
		$option->Render("body");
?>
...
?>
<form name="fberitaview" id="fberitaview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($berita_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $berita_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="berita">
<input type="hidden" name="modal" value="<?php echo intval($berita_view->IsModal) ?>">
<table class="table table-striped table-bordered table-hover table-condensed ewViewTable">
<?php if ($berita->id->Visible) { // id ?>
	<tr id="r_id">
		<td class="col-sm-2"><span id="elh_berita_id"><?php echo $berita->id->FldCaption() ?></span></td>
		<td data-name="id"<?php echo $berita->id->CellAttributes() ?>>
<span id="el_berita_id">
<span<?php echo $berita->id->ViewAttributes() ?>>
<?php echo $berita->id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($berita->foto_berita->Visible) { // foto_berita ?>
	<tr id="r_foto_berita">
		<td class="col-sm-2"><span id="elh_berita_foto_berita"><?php echo $berita->foto_berita->FldCaption() ?></span></td>
...
<?php echo ew_GetFileViewTag($berita->foto_berita, $berita->foto_berita->ViewValue) ?>
</span>
</span>
</td>
	</tr>
...
<?php if ($berita->judul_berita->Visible) { // judul_berita ?>
	<tr id="r_judul_berita">
		<td class="col-sm-2"><span id="elh_berita_judul_berita"><?php echo $berita->judul_berita->FldCaption() ?></span></td>
...
<?php echo $berita->judul_berita->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($berita->deskripsi->Visible) { // deskripsi ?>
	<tr id="r_deskripsi">
		<td class="col-sm-2"><span id="elh_berita_deskripsi"><?php echo $berita->deskripsi->FldCaption() ?></span></td>
...
<?php echo $berita->deskripsi->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
