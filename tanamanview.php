...
$tanaman_view = NULL; // Initialize page object first

class ctanaman_view extends ctanaman {
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["id"] <> "") {
				...
				$sReturnUrl = "tanamanlist.php"; // Return to list
			}
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "tanamanlist.php"; // No matching record, return to list
					}
			}
		} else {
			$sReturnUrl = "tanamanlist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
...
// Create page object
if (!isset($tanaman_view)) $tanaman_view = new ctanaman_view();

// Page init
$tanaman_view->Page_Init();

// Page main
$tanaman_view->Page_Main();

// Global Page Rendering event (in userfn*.php)
Page_Rendering();

// Page Rendering event
$tanaman_view->Page_Render();
?>
...
var CurrentForm = ftanamanview = new ew_Form("ftanamanview", "view");

// Form_CustomValidate event
ftanamanview.Form_CustomValidate = 
...
ftanamanview.ValidateRequired = <?php echo json_encode(EW_CLIENT_VALIDATE) ?>;
...
<div class="ewToolbar">
<?php $tanaman_view->ExportOptions->Render("body") ?>
<?php
	foreach ($tanaman_view->OtherOptions as &$option)
		$option->Render("body");
?>
<div class="clearfix"></div>
</div>
<?php $tanaman_view->ShowPageHeader(); ?>
<?php
$tanaman_view->ShowMessage();
?>
<form name="ftanamanview" id="ftanamanview" class="form-inline ewForm ewViewForm" action="<?php echo ew_CurrentPage() ?>" method="post">
<?php if ($tanaman_view->CheckToken) { ?>
<input type="hidden" name="<?php echo EW_TOKEN_NAME ?>" value="<?php echo $tanaman_view->Token ?>">
<?php } ?>
<input type="hidden" name="t" value="tanaman">
<input type="hidden" name="modal" value="<?php echo intval($tanaman_view->IsModal) ?>">
<table class="table table-striped table-bordered table-hover table-condensed ewViewTable">
<?php if ($tanaman->id->Visible) { // id ?>
	<tr id="r_id">
		<td class="col-sm-2"><span id="elh_tanaman_id"><?php echo $tanaman->id->FldCaption() ?></span></td>
		<td data-name="id"<?php echo $tanaman->id->CellAttributes() ?>>
<span id="el_tanaman_id">
<span<?php echo $tanaman->id->ViewAttributes() ?>>
<?php echo $tanaman->id->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($tanaman->foto_tanaman->Visible) { // foto_tanaman ?>
	<tr id="r_foto_tanaman">
		<td class="col-sm-2"><span id="elh_tanaman_foto_tanaman"><?php echo $tanaman->foto_tanaman->FldCaption() ?></span></td>
		<td data-name="foto_tanaman"<?php echo $tanaman->foto_tanaman->CellAttributes() ?>>
<span id="el_tanaman_foto_tanaman">
<span<?php echo $tanaman->foto_tanaman->ViewAttributes() ?>>
<?php echo ew_GetFileViewTag($tanaman->foto_tanaman, $tanaman->foto_tanaman->ViewValue) ?>
</span>
</span>
</td>
	</tr>
...
<?php if ($tanaman->nama_ilmiah->Visible) { // nama_ilmiah ?>
	<tr id="r_nama_ilmiah">
		<td class="col-sm-2"><span id="elh_tanaman_nama_ilmiah"><?php echo $tanaman->nama_ilmiah->FldCaption() ?></span></td>
		<td data-name="nama_ilmiah"<?php echo $tanaman->nama_ilmiah->CellAttributes() ?>>
<span id="el_tanaman_nama_ilmiah">
<span<?php echo $tanaman->nama_ilmiah->ViewAttributes() ?>>
<?php echo $tanaman->nama_ilmiah->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($tanaman->nama_lokal->Visible) { // nama_lokal ?>
	<tr id="r_nama_lokal">
		<td class="col-sm-2"><span id="elh_tanaman_nama_lokal"><?php echo $tanaman->nama_lokal->FldCaption() ?></span></td>
		<td data-name="nama_lokal"<?php echo $tanaman->nama_lokal->CellAttributes() ?>>
<span id="el_tanaman_nama_lokal">
<span<?php echo $tanaman->nama_lokal->ViewAttributes() ?>>
<?php echo $tanaman->nama_lokal->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
<?php if ($tanaman->famili_tanaman->Visible) { // famili_tanaman ?>
	<tr id="r_famili_tanaman">
		<td class="col-sm-2"><span id="elh_tanaman_famili_tanaman"><?php echo $tanaman->famili_tanaman->FldCaption() ?></span></td>
		<td data-name="famili_tanaman"<?php echo $tanaman->famili_tanaman->CellAttributes() ?>>
<span id="el_tanaman_famili_tanaman">
<span<?php echo $tanaman->famili_tanaman->ViewAttributes() ?>>
<?php echo $tanaman->famili_tanaman->ViewValue ?></span>
</span>
</td>
	</tr>
<?php } ?>
