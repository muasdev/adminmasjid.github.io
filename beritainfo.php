...
class cberita extends cTable {
	...
	function __construct() {
		...
		// id
		$this->id = new cField('berita', 'berita', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		$this->id->Sortable = TRUE; // Allow sort
		$this->id->FldDefaultErrMsg = $Language->Phrase("IncorrectInteger");
		$this->fields['id'] = &$this->id;

		// foto_berita
		$this->foto_berita = new cField('berita', 'berita', 'x_foto_berita', 'foto_berita', '`foto_berita`', '`foto_berita`', 201, -1, TRUE, '`foto_berita`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'FILE');
		$this->foto_berita->Sortable = TRUE; // Allow sort
		$this->fields['foto_berita'] = &$this->foto_berita;

		// judul_berita
		$this->judul_berita = new cField('berita', 'berita', 'x_judul_berita', 'judul_berita', '`judul_berita`', '`judul_berita`', 201, -1, FALSE, '`judul_berita`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->judul_berita->Sortable = TRUE; // Allow sort
		$this->fields['judul_berita'] = &$this->judul_berita;

		// deskripsi
		$this->deskripsi = new cField('berita', 'berita', 'x_deskripsi', 'deskripsi', '`deskripsi`', '`deskripsi`', 201, -1, FALSE, '`deskripsi`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	function getSqlFrom() { // From
		return ($this->_SqlFrom <> "") ? $this->_SqlFrom : "`berita`";
	}
	...
	function GetViewUrl($parm = "") {
		if ($parm <> "")
			$url = $this->KeyUrl("beritaview.php", $this->UrlParm($parm));
		else
			$url = $this->KeyUrl("beritaview.php", $this->UrlParm(EW_TABLE_SHOW_DETAIL . "="));
	...
	function GetAddUrl($parm = "") {
		if ($parm <> "")
			$url = "beritaadd.php?" . $this->UrlParm($parm);
		else
			$url = "beritaadd.php";
		return $this->AddMasterUrl($url);
	}
	...
	function GetEditUrl($parm = "") {
		$url = $this->KeyUrl("beritaedit.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetCopyUrl($parm = "") {
		$url = $this->KeyUrl("beritaadd.php", $this->UrlParm($parm));
		return $this->AddMasterUrl($url);
	}
	...
	function GetDeleteUrl() {
		return $this->KeyUrl("beritadelete.php", $this->UrlParm());
	}
	...
	function LoadListRowValues(&$rs) {
		...
		$this->foto_berita->Upload->DbValue = $rs->fields('foto_berita');
		$this->judul_berita->setDbValue($rs->fields('judul_berita'));
	...
	function RenderEditRow() {
		...
		$this->judul_berita->EditValue = $this->judul_berita->CurrentValue;
		...
		if (!$Doc->ExportCustom) {
			...
			if ($Doc->Horizontal) { // Horizontal format, write header
				...
				if ($ExportPageType == "view") {
					...
					if ($this->foto_berita->Exportable) $Doc->ExportCaption($this->foto_berita);
					if ($this->judul_berita->Exportable) $Doc->ExportCaption($this->judul_berita);
			...
			if (intval($RecCnt) >= intval($StartRec)) {
				...
				if (!$Doc->ExportCustom) {
					...
					if ($ExportPageType == "view") {
						...
						if ($this->foto_berita->Exportable) $Doc->ExportField($this->foto_berita);
						if ($this->judul_berita->Exportable) $Doc->ExportField($this->judul_berita);
